...
- `mbes_tools.water_column_geo` — true **geo-referenced** water-column products (console script `mbes-wc-mosaic`): `NavTrack` + `georeference_frame` place each sample at `(easting, northing, depth)` (local ENU or UTM), a streaming `GeoMosaic` builds a plan-view coverage map (peak-hold or intensity-mean, optional `--depth-band` absolute-depth or `--altitude-band` height-above-seafloor midwater/plume filter), a multi-line `--combine` composite shares one anchor/CRS (optionally across a process pool, `--workers N`, with a bit-identical result), vessel attitude (roll/pitch/heave) is applied with the beam fan left receive-stabilized, and `--normalize empirical` removes per-range + per-beam-angle acquisition gain and `--normalize sv` (+ `--absorption`) re-expresses amplitude as relative volume-scattering Sv — undoing the applied `X·logR` TVG and re-applying the `20·logR` volume law (both relative dB, not absolutely calibrated) — `--clean-water` keeps only the bottom-sidelobe-free water column (samples inside the minimum bottom-detect slant range), and `--fill-nodata N` interpolates the small empty cells left by a thin selection or fine grid (distance-bounded, numpy-only; filled cells keep count 0). Does **not** assume the WC file carries nav — `resolve_nav_track` auto-discovers a same-stem `.kmall`/`.all` companion (prefers `#SKM`) and a coverage guard skips pings outside the nav span. pyproj optional (auto-UTM without it). Export the mosaic as a georeferenced **GeoTIFF** (`--geotiff`, needs `--projector utm` + the `geo` extra) or a numpy-only **ESRI ASCII Grid** (`--asc`, `.asc` + `.prj`). Verified on real EM124/EM122/EM304 matched pairs
